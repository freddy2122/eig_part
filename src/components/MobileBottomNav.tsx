  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 md:hidden">
      <div className="mx-auto max-w-md px-4 pb-3">
        <div className="grid grid-cols-3 rounded-2xl border border-eig-yellow/40 bg-white/95 p-1 shadow-[0_8px_30px_rgba(11,46,122,0.12)] backdrop-blur">
          {items.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex flex-col items-center gap-1 rounded-xl py-2 text-[11px] font-semibold transition-all duration-250 active:scale-95 ${
                  item.active ? "bg-eig-yellow-light text-eig-blue" : "text-slate-600"
                }`}
              >
